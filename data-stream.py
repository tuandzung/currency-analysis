...
def process_stream(stream, kafka_producer, target_topic):
    ...
    def pair(data):
        ...
        return record.get('symbol'), (float(record.get('price')), float(record.get('base_vol')), 1)

    stream.map(pair).reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1], a[2] + b[2])).map(
        lambda kv: (kv[0], kv[1][0] / kv[1][2], kv[1][1] * kv[1][0] / kv[1][2])).foreachRDD(send_to_kafka)
